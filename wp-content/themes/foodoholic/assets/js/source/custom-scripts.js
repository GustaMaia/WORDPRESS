...
jQuery(function($){
    ...
    ( function() {
        ...
        menuToggle.on( 'click.foodoholic', function() {
            // jscs:disable
    ...
    ( function() {
        ...
        function toggleFocusClassTouchScreen() {
            if ( window.innerWidth >= 910 ) {
                $( document.body ).on( 'touchstart.foodoholic', function( e ) {
                    if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
                        $( '.main-navigation li' ).removeClass( 'focus' );
                    }
                } );
                siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.foodoholic', function( e ) {
                ...
                siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).unbind( 'touchstart.foodoholic' );
            }
        }

        if ( 'ontouchstart' in window ) {
            $( window ).on( 'resize.foodoholic', toggleFocusClassTouchScreen );
            toggleFocusClassTouchScreen();
        }

        siteNavigation.find( 'a' ).on( 'focus.foodoholic blur.foodoholic', function() {
    ...
    $(window).on( 'load resize', function() {
        ...
        var thumbHeight = $('.services-section .post-thumbnail').height();

        if ( $(window).width() > 567 ) {
            $('.services-section .section-content-wrapper.layout-two .entry-container, .services-section .section-content-wrapper.layout-four .entry-container').css('height', thumbHeight);
        } else {
            $('.services-section .section-content-wrapper.layout-two .entry-container, .services-section .section-content-wrapper.layout-four .entry-container').removeAttr('style');
        }
    ...
    $(window).on( 'load resize', function() {
        ...
        if ( $(window).width() > 666 ) {
            $('.services-section .section-content-wrapper.layout-three .entry-container').css('height', thumbHeight);
        } else {
            $('.services-section .section-content-wrapper.layout-three .entry-container').removeAttr('style');
        }
    });
